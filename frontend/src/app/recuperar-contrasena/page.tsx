...
function RecuperarContrasenaContent() {
  ...
  const rules = [
    ...
    { label: 'Una minúscula', ok: /[a-z]/.test(passwordValue) },
    { label: 'Un símbolo', ok: /[^A-Za-z0-9]/.test(passwordValue) },
  ]
              ...
              <div className="space-y-1.5">
                ...
                {passwordValue.length > 0 && (
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 pt-1">
                    {rules.map((r) => (
                      <span key={r.label} className={cn('flex items-center gap-1 text-xs', r.ok ? 'text-emerald-600' : 'text-muted-foreground')}>
                        <Check className={cn('h-3 w-3 shrink-0', r.ok ? 'opacity-100' : 'opacity-30')} />
                        {r.label}
                      </span>
                    ))}
                  </div>
