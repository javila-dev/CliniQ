...
function ProcedimientosTable({
  ...
  // Con el filtro por procedimiento, uno sin profesionales asociados no se puede agendar.
          ...
          {servicios.map((s) => (
            <tr key={s.id} className={cn('hover:bg-gray-50/50 transition-colors', !s.activo && 'opacity-55')}>
              <td className="px-4 py-2">
                ...
                {filtroActivo && s.activo && (s.profesionales_detalle?.length ?? 0) === 0 && (
                  <button
                    type="button"
                    onClick={puedeGestionar ? () => onEdit(s) : undefined}
                    disabled={!puedeGestionar}
                    title={puedeGestionar
                      ? 'Sin profesionales asociados no se puede agendar. Abre el procedimiento para asignarlos.'
                      : 'Sin profesionales asociados no se puede agendar.'}
                    className="mt-1 inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[10px] font-medium text-amber-800 enabled:hover:bg-amber-100 transition-colors"
                  >
                    Falta: profesional
                  </button>
                )}
...
export function ProcedimientosCatalogo() {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          Lo que ofreces en una cita: duración, precio, consentimientos y zonas.
        </p>
        {puedeGestionar && (
      ...
      <ConfirmDialog
        ...
        description={
          <>
            Se eliminará <strong>{deleteTarget?.nombre}</strong> junto con sus zonas y consentimientos
