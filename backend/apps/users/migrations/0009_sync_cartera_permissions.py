...
def sync_cartera_permissions(apps, schema_editor):
    ...
    for rol in Rol.objects.filter(activo=True, slug__in=["admin", "profesional", "recepcion"]):
        claves = {"cartera.ver"}
        if rol.slug in {"admin", "recepcion"}:
            claves.add("cartera.registrar_pago")
        for clave in claves:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("users", "0008_sync_cotizaciones_permissions"),
    ]

    operations = [
        migrations.RunPython(sync_cartera_permissions, migrations.RunPython.noop),
    ]
